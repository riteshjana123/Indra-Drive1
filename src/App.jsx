...
function Controls({ running, setRunning, scenario, setScenario, speed, setSpeed, restart, syncControl, selectedObstacleType, setSelectedObstacleType }) {
  ...
  return <aside className="panel controls-panel">
    ...
    <div className="obstacle-palette">
      ...
      <div className="palette-note">Double-click a placed obstacle to remove it. Objects leave the scene permanently after passing the car.</div>
    </div>
...
function RoadCanvas({ running, speed, selectedObstacleType, setSelectedObstacleType, obstacles, setObstacles, crashed, setCrashed }) {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      const nextOffset = roadOffsetRef.current + (delta / 1000) * currentSpeed * 2.4
      ...
      if (surviving.length !== current.length) setObstacles(surviving)
      if (collision) setCrashed(true)
      frame = window.requestAnimationFrame(tick)
    }
    frame = window.requestAnimationFrame(tick)
    return () => window.cancelAnimationFrame(frame)
  }, [running, crashed, setObstacles, setCrashed])
  ...
  const placeObstacle = (clientX, clientY, type) => {
    ...
    const obstacle = { id: `${type}-${Date.now()}-${Math.random().toString(16).slice(2)}`, type, x, y, worldY: y - roadOffsetRef.current }
  ...
  const stripeY = Array.from({ length: 10 }, (_, index) => index * 78 + (roadOffset % 78) - 78)
  ...
  return <div className={`twin-stage ${crashed ? 'stage-crashed' : ''}`}>
    ...
    <div className={`drop-area ${dragActive ? 'drag-active' : ''}`} onDragEnter={(event) => { event.preventDefault(); setDragActive(true) }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'copy'; setDragActive(true) }} onDragLeave={() => setDragActive(false)} onDrop={handleDrop}>
      <svg ref={sceneRef} className="road-scene" viewBox="0 0 760 560" role="img" aria-label="Top-down perfectly straight road with fixed blue ego vehicle, moving environment and small draggable obstacles" onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'copy' }} onDrop={handleDrop} onClick={handleRoadClick}>
...
function App() {
  ...
  useEffect(() => { setTelemetry((current) => ({ ...current, speed, targetSpeed: speed, acceleration: 0, steering: 0 })) }, [speed])

  const syncControl = (control) => {
    updateSimulationControl(control).catch(() => undefined)
  }
  ...
  const decision = useMemo(() => crashed
    ? 'Collision detected. INDRA stopped the simulation because a placed obstacle touched the ego vehicle.'
    : obstacles.length
