...
function ObstacleIcon({ type, small = false }) {
  ...
  if (type === 'bike') return <svg className={cls} width={size} height={size} viewBox="0 0 32 32"><circle cx="8" cy="23" r="5"/><circle cx="24" cy="23" r="5"/><path d="M8 23l6-12 5 12m-7-7h7m-5-5 3 12M14 11l-4 0"/></svg>
...
function Controls({ running, setRunning, speed, setSpeed, restart, syncControl, selectedObstacleType, setSelectedObstacleType, obstacles }) {
  ...
  return <aside className="panel controls-panel">
    ...
    <div className="control-section"><label htmlFor="scenario">Scenario type</label><select id="scenario" defaultValue="Straight road"><option>Straight road</option><option>Curved road</option><option>Two-way road</option></select></div>
    <div className="control-section"><div className="label-row"><label>Traffic density</label><strong>{trafficLabel} · {trafficPercent}%</strong></div><div className="density-bars">{[0,1,2].map(i => <b key={i} style={{ opacity: i < activeBars ? 1 : .32 }} />)}</div><p className="muted">Active road objects increase traffic density · {obstacles.length} object{obstacles.length === 1 ? '' : 's'}</p></div>
...
function EgoVehicle({ x, rotation }) {
  return <g className="ego-vehicle-blue ego-realistic ego-fixed ego-maneuvering" transform={`translate(${x} ${EGO_Y}) rotate(${rotation})`}>
...
function RoadCanvas({ running, speed, selectedObstacleType, setSelectedObstacleType, obstacles, setObstacles, crashed, setCrashed, setTelemetry, maneuver, setManeuver }) {
  ...
  const roadCandidates=Array.from({length:31},(_,i)=>ROAD_MIN_X+((ROAD_MAX_X-ROAD_MIN_X)/30)*i)

  const isPathClear=(candidateX,referenceY=EGO_Y-20,ignoreId=null)=>!obstaclesRef.current.some(item=>{
    if(item.id===ignoreId)return false
    const y=Number(item.worldY)+offsetRef.current
    if(y<referenceY-90||y>EGO_Y+100)return false
    return Math.abs(Number(item.x)-candidateX)<EGO_HALF_WIDTH+radiusOf(item.type)+18
  })
  ...
  const bestSteeringTarget=threat=>{
    ...
    const nearby=obstaclesRef.current.filter(item=>item.id!==threat.id).map(item=>({item,y:Number(item.worldY)+offsetRef.current,r:radiusOf(item.type)})).filter(({y})=>y>threat.movedY-100&&y<EGO_Y+100)
    const score=x=>{
      const obstacleClearance=nearby.length?Math.min(...nearby.map(({item,r})=>Math.abs(Number(item.x)-x)-(r+EGO_HALF_WIDTH))):160
      ...
      return Math.min(obstacleClearance,threatClearance)*4-movementCost*.55-centerCost*.08
    }
  ...
  const findThreat=nextOffset=>{
    ...
    for(const item of obstaclesRef.current){
      ...
      const lateral=EGO_HALF_WIDTH+radiusOf(item.type)+45
      if(y<EGO_Y-18&&y>EGO_Y-WARNING_DISTANCE&&Math.abs(Number(item.x)-egoXRef.current)<lateral){
  ...
  useEffect(()=>{
    ...
    const tick=now=>{
      ...
      if(v>0&&threat&&(m.mode==='cruise'||m.obstacleId!==threat.id||!isPathClear(m.targetX,threat.movedY,threat.id))){
        const target=bestSteeringTarget(threat)
        m=target===null?{mode:'brake',targetX:egoXRef.current,reason:'BRAKING — ROAD FULL',obstacleId:threat.id}:{mode:threat.type==='car'?'overtake':'avoid',targetX:target,reason:threat.type==='car'?'OVERTAKING AHEAD':'STEERING AROUND OBSTACLE',obstacleId:threat.id}
        maneuverRef.current=m
        setManeuver(m)
      }else if((m.mode==='avoid'||m.mode==='overtake')&&(!tracked||trackedY>EGO_Y+80)){
        const returnTarget=isPathClear(ROAD_CENTER_X,EGO_Y,null)?ROAD_CENTER_X:bestSteeringTarget({x:egoXRef.current,movedY:EGO_Y-10,id:'virtual',type:'hazard'})
        const target=returnTarget??egoXRef.current
        m=Math.abs(target-egoXRef.current)<2?{mode:'cruise',targetX:ROAD_CENTER_X,reason:'OPEN ROAD',obstacleId:null}:{mode:'return',targetX:target,reason:'CLEARING THE ROAD',obstacleId:null}
        maneuverRef.current=m
        setManeuver(m)
      }
      ...
      const lateralVelocity=delta>0?(lateralDelta/delta)*1000:0
      const desiredHeading=Math.max(-MAX_STEERING_ANGLE,Math.min(MAX_STEERING_ANGLE,lateralVelocity*.055))
      headingRef.current+=(desiredHeading-headingRef.current)*.22
      if(Math.abs(headingRef.current)<.12)headingRef.current=0
      setHeading(headingRef.current)

      if(v>0&&now-lastTrailRef.current>45){
        const nextTrail=[{x:nextX,y:EGO_Y+43},...trailRef.current.map((p,i)=>({x:p.x,y:EGO_Y+53+i*10}))].slice(0,TRAIL_LENGTH)
      ...
      for(const item of current){
        ...
        if(dx<=EGO_HALF_WIDTH+radiusOf(item.type)&&dy<=EGO_HALF_HEIGHT+radiusOf(item.type)){collision=true;break}
      }
      ...
      const steeringValue=Math.max(-MAX_STEERING_ANGLE,Math.min(MAX_STEERING_ANGLE,Math.round(headingRef.current)))
      setTelemetry(t=>({...t,speed:v,targetSpeed:v,acceleration:m.mode==='brake'&&v>0?-3.2:Math.abs(lateralDelta)>.1?.8:0,steering:steeringValue,ttc:Math.max(.1,Math.min(8,ttc)),collisionProbability:m.mode==='brake'?Math.min(99,Math.max(20,100-ttc*20)):threat?Math.min(80,Math.max(5,80-ttc*8)):0}))

      if(m.mode==='return'&&Math.abs(m.targetX-nextX)<2){const cruise={mode:'cruise',targetX:ROAD_CENTER_X,reason:'OPEN ROAD',obstacleId:null};maneuverRef.current=cruise;setManeuver(cruise)}
  ...
  useEffect(()=>{
    if(speed<=0){trailRef.current=[];setTrail([]);headingRef.current=0;setHeading(0)}
  },[speed])
  ...
  const handleClick=e=>{if(!selectedObstacleType||crashed)return;if(e.target.closest?.('.road-obstacle'))return;placeObstacle(e.clientX,e.clientY,selectedObstacleType)}
  const stripeOffset=roadOffset%78
  const stripeY=Array.from({length:10},(_,i)=>i*78+stripeOffset-78)
  const vegOffset=roadOffset%620
  ...
  return <div className={`twin-stage ${crashed?'stage-crashed':''}`}>
    ...
    <div className={`drop-area ${dragActive?'drag-active':''}`} onDragEnter={e=>{e.preventDefault();setDragActive(true)}} onDragOver={e=>{e.preventDefault();e.dataTransfer.dropEffect='copy';setDragActive(true)}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setDragActive(false)}} onDrop={handleDrop}>
      <svg ref={sceneRef} className="road-scene" viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} aria-label="Straight road with open-road autonomous steering" onDragOver={e=>{e.preventDefault();e.dataTransfer.dropEffect='copy'}} onDrop={handleDrop} onClick={handleClick}>
        <defs>
        ...
        <rect x={ROAD_LEFT-2} width="2" height={VIEW_H} className="road-edge"/><rect x={ROAD_RIGHT} width="2" height={VIEW_H} className="road-edge"/>
        {[-620,0,620].map((base,i)=><g key={`veg-${i}`} className="moving-environment" transform={`translate(0 ${base+vegOffset})`}><Vegetation/></g>)}
        {stripeY.map((y,i)=><rect key={i} x="378" y={y} width="4" height="42" rx="2" className="road-centerline"/>)}
        <RoadTrail points={trail}/>
        ...
        <EgoVehicle x={egoX} rotation={heading}/>
        {crashed&&<g className="collision-marker" transform={`translate(${egoX} ${EGO_Y}) rotate(${heading})`}><circle r="34"/><path d="M-9-9l18 18m0-18-18 18"/></g>}
      </svg>
...
function VehicleStatus({ telemetry, obstacles, crashed, maneuver }) {
  const acceleration=Number(telemetry.acceleration),steering=Number(telemetry.steering),ttc=Number(telemetry.ttc),probability=Number(telemetry.collisionProbability)
  return <aside className="panel status-panel"><Title eyebrow="Telemetry" title="Vehicle status" action="EGO-01"/><div className="telemetry-grid">{[['CURRENT SPEED',`${Math.round(Number(telemetry.speed))} km/h`,'neutral'],['TARGET SPEED',`${Math.round(Number(telemetry.targetSpeed))} km/h`,'teal'],['ACCELERATION',`${acceleration.toFixed(1)} m/s²`,'amber'],['STEERING ANGLE',`${Math.round(steering)}°`,'neutral']].map(([label,value,tone])=><div className="telemetry-card" key={label}><span>{label}</span><strong className={`value-${tone}`}>{value}</strong></div>)}</div><div className="status-row"><span>Brake status</span><strong className={`brake-status ${crashed||maneuver.mode==='brake'?'brake-danger':'clear-brake'}`}><i/>{crashed?' LOCKED':maneuver.mode==='brake'?' APPLIED':' RELEASED'}</strong></div><div className="risk-readout"><div><span>TIME TO COLLISION</span><strong className={crashed?'danger-value':''}>{crashed?'0.0':ttc.toFixed(1)}<small> sec</small></strong></div><div><span>COLLISION PROBABILITY</span><strong className={crashed?'danger-value':''}>{crashed?'100':Math.round(probability)}<small>%</small></strong></div></div><div className="risk-meter"><span className={crashed?'risk-full':''} style={{width:`${crashed?100:Math.round(probability)}%`}}/></div><div className="current-risk"><span>Current risk</span><strong className={crashed?'collision-risk':maneuver.mode==='cruise'?'clear-risk':'watch-risk'}><i/>{crashed?' COLLISION':maneuver.mode==='brake'?' HIGH RISK':maneuver.mode==='overtake'?' OVERTAKING':maneuver.mode==='avoid'||maneuver.mode==='return'?' MANEUVER':obstacles.length?' MONITOR':' CLEAR'}</strong></div><div className="selected-path"><span>Selected path</span><strong>{maneuver.mode==='overtake'?'Free-road passing path':maneuver.mode==='avoid'?'Free steering path':maneuver.mode==='return'?'Center recovery path':maneuver.mode==='brake'?'Safe stopping path':'Open road'} <em>INDRA</em></strong></div><div className="status-obstacles"><span>Active obstacles</span><strong>{obstacles.length}</strong></div></aside>
}
...
function App(){
  const [running,setRunning]=useState(true),[speed,setSpeed]=useState(38),[telemetry,setTelemetry]=useState({...initialTelemetry}),[obstacles,setObstacles]=useState([]),[crashed,setCrashed]=useState(false),[selectedObstacleType,setSelectedObstacleType]=useState(null),[maneuver,setManeuver]=useState({mode:'cruise',targetX:ROAD_CENTER_X,reason:'OPEN ROAD',obstacleId:null})
  useEffect(()=>{getSimulationState().then(state=>{setRunning(Boolean(state.running));const value=Number(state.vehicleSpeed??state.telemetry?.targetSpeed??38);setSpeed(Number.isFinite(value)?Math.max(0,Math.min(MAX_SPEED,Math.round(value))):38);if(state.telemetry)setTelemetry(t=>({...t,...state.telemetry}))}).catch(()=>undefined)},[])
  ...
  const restart=()=>{setRunning(true);setSpeed(38);setTelemetry({...initialTelemetry});setObstacles([]);setCrashed(false);setSelectedObstacleType(null);setManeuver({mode:'cruise',targetX:ROAD_CENTER_X,reason:'OPEN ROAD',obstacleId:null});restartSimulation().catch(()=>undefined)}
  const decision=useMemo(()=>{if(crashed)return'Collision detected. INDRA stopped the vehicle because an obstacle reached the ego vehicle.';if(maneuver.mode==='overtake')return'A vehicle entered the driving corridor. INDRA steers around it with a continuous lateral path, passes it, then recovers toward the center.';if(maneuver.mode==='avoid')return'An obstacle entered the driving corridor. INDRA is continuously steering around it instead of switching between fixed lanes.';if(maneuver.mode==='return')return'The obstacle has been passed. INDRA is smoothly steering back toward the preferred center position.';if(maneuver.mode==='brake')return'The road is occupied across the available steering area. INDRA is braking instead of forcing an unsafe pass.';return'The road is clear. INDRA is cruising while the road environment moves toward the fixed vehicle.'},[crashed,maneuver.mode])
  return <main className="app-shell"><header className="app-header"><div className="brand-lockup"><img className="brand-mark" src={logo} alt="INDRA-DRIVE logo"/><div><h1>INDRA-<b>DRIVE</b></h1><p>Predictive Risk-Adaptive Path Planning <span>/</span> Realistic Road Digital Twin</p></div></div><div className="header-status"><Pill>SYSTEM STATUS: ACTIVE</Pill><Pill>SIMULATION: {crashed?'STOPPED':running?'RUNNING':'PAUSED'}</Pill><Pill tone="blue">MODE: INDRA ADAPTIVE</Pill></div></header><div className="dashboard-grid"><Controls running={running&&!crashed} setRunning={value=>{if(!crashed)setRunning(value)}} speed={speed} setSpeed={setSpeed} restart={restart} syncControl={syncControl} selectedObstacleType={selectedObstacleType} setSelectedObstacleType={setSelectedObstacleType} obstacles={obstacles}/><section className="panel twin-panel"><Title eyebrow="Live environment / top-down view" title="Digital twin" action="30 FPS"/><RoadCanvas running={running&&speed>0} speed={speed} selectedObstacleType={selectedObstacleType} setSelectedObstacleType={setSelectedObstacleType} obstacles={obstacles} setObstacles={setObstacles} crashed={crashed} setCrashed={setCrashed} setTelemetry={setTelemetry} maneuver={maneuver} setManeuver={setManeuver}/><div className="twin-insight"><span className={`alert-icon ${crashed?'collision-icon':maneuver.mode==='cruise'?'clear-icon':'watch-icon'}`}>{crashed?'!':maneuver.mode==='cruise'?'✓':'!'}</span><div><strong className={crashed?'collision-text':maneuver.mode==='cruise'?'clear-text':'watch-text'}>{crashed?'COLLISION DETECTED':maneuver.mode==='overtake'?'OVERTAKING':maneuver.mode==='avoid'?'OBSTACLE AVOIDANCE':maneuver.mode==='return'?'RETURNING TO ROAD':maneuver.mode==='brake'?'BRAKING':'OPEN ROAD'}</strong><p>{decision}</p></div><span className="insight-time">{crashed?'STOP':maneuver.mode==='cruise'?'SAFE':'ACTIVE'}</span></div></section><VehicleStatus telemetry={telemetry} obstacles={obstacles} crashed={crashed} maneuver={maneuver}/></div><div className="lower-grid"><Performance speed={speed} crashed={crashed}/><section className="panel decision-panel"><Title eyebrow="Why did INDRA act?" title="Explainable decision" action="AUTO-LOGGED"/><div className="decision-copy"><div className="decision-tag"><span>01</span> DECISION TRACE</div><p>{decision}</p><div className="decision-facts"><div><span>PATHS EVALUATED</span><strong>{maneuver.mode==='cruise'?'31':maneuver.mode==='brake'?'0':'31'}</strong></div><div><span>RISK REDUCTION</span><strong>{crashed?'0%':maneuver.mode==='brake'?'35%':'100%'}</strong></div><div><span>WEIGHT PROFILE</span><strong>SAFETY <em>70%</em></strong></div></div><div className="action-callout"><span>↳</span><div><small>ACTION</small><strong>{crashed?'Stop vehicle + log collision':maneuver.mode==='overtake'?'Steer around vehicle + pass + recover':maneuver.mode==='avoid'?'Steer around obstacle + maintain clearance':maneuver.mode==='return'?'Recover preferred road position':maneuver.mode==='brake'?'Brake + wait for safe gap':`Cruise at ${Math.round(speed)} km/h`}</strong></div></div></div></section></div><footer className="app-footer"><span><b className="live-dot"/> SIMULATION ENGINE READY</span><span>SCENARIO: STRAIGHT ROAD &nbsp;·&nbsp; WEATHER: CLEAR &nbsp;·&nbsp; SPEED LIMIT: 100 km/h</span><strong>SAFETY FIRST <i>◆</i></strong></footer></main>
}
export default App
