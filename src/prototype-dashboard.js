...
const heartbeat = async () => {
  try { await fetch(`${API}/prototype/heartbeat`, { method: 'POST' }) } catch { /* keep the last known UI state */ }
}
...
function render(state, panel) {
  ...
  panel.innerHTML = `
    <div class="panel-title prototype-panel-title"><div><span class="eyebrow">Prototype readiness / live safety core</span><h2>Vehicle systems</h2></div><span class="panel-action">LIVE</span></div>
    ...
    <div class="prototype-layout">
      <div class="prototype-components"><div class="prototype-section-heading"><span class="eyebrow">Component health</span><span>${health.filter(([,v]) => v === 'READY').length}/${health.length} READY</span></div><div class="component-grid">${health.map(([name,value]) => `<div class="component-chip component-${statusClass(value)}"><span>${esc(name)}</span><strong>${esc(value)}</strong></div>`).join('')}</div></div>
      <div class="prototype-safety"><div class="prototype-section-heading"><span class="eyebrow">Safety interlocks</span><span>${reasons.length ? `${reasons.length} ACTIVE` : 'CLEAR'}</span></div><div class="safety-list">${(reasons.length ? reasons : ['EMERGENCY STOP READY','CHARGER INTERLOCK READY','WATCHDOG HEARTBEAT READY']).map(item => `<div><i></i><span>${esc(item)}</span></div>`).join('')}</div></div>
  ...
  panel.querySelector('[data-proto="estop"]')?.addEventListener('click', async () => { await postControl({ estop: !state.estop }).then(() => refresh(panel)).catch(() => {}) })
  panel.querySelector('[data-proto="sensor"]')?.addEventListener('click', async () => { await postControl({ sensorHealth: state.sensorsHealthy === false }).then(() => refresh(panel)).catch(() => {}) })
  panel.querySelector('[data-proto="compute"]')?.addEventListener('click', async () => { await postControl({ computeHealth: state.computeHealthy === false }).then(() => refresh(panel)).catch(() => {}) })
  panel.querySelector('[data-proto="actuator"]')?.addEventListener('click', async () => { await postControl({ actuatorHealth: state.actuatorHealthy === false }).then(() => refresh(panel)).catch(() => {}) })
